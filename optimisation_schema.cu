%%writefile optimisation_schema.cu
...
// Noyau CUDA avec mémoire partagée optimisée
__global__ void matrixMulSharedMemoryOptimized(const float *A, const float *B, float *C, int n) {
...
int main() {
    ...
    matrixMulSharedMemoryOptimized<<<gridDim, blockDim>>>(d_A, d_B, d_C, N);

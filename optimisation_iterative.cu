%%writefile optimisation_iterative.cu
...
// Noyau optimisé avec déroulement de boucles
__global__ void matrixMulUnrolled(const float *A, const float *B, float *C, int n) {
    __shared__ float A_shared[32][32 + 1];
...
int main() {
    ...
    // Essayez différentes tailles de blocs
    dim3 blockDim(32, 32); // Taille initiale de bloc
    dim3 gridDim((N + blockDim.x - 1) / blockDim.x, (N + blockDim.y - 1) / blockDim.y);

    matrixMulUnrolled<<<gridDim, blockDim>>>(d_A, d_B, d_C, N);
